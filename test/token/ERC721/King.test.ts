...
import { King } from "@orivium/types";
...
describe("King ERC721", () => {
  ...
  beforeEach(async () => {
    ...
    ogMerkleTree = new MerkleTree(ogLeafNodes, ethers.keccak256, {
      sortPairs: true,
    });
    ...
    king = <King>await kingFactory.deploy(rootHash, ogRootHash, multiSigWallet);
  });
  ...
  describe("sale", async () => {
    describe("before sale opens", async () => {
      describe("single purchase", async () => {
        it("should revert open before sale opens", async () => {
          await expect(
            king.connect(purchaser).purchase({ value: priceEth })
          ).to.be.revertedWithCustomError(king, "SalePhaseNotStarted");
        });
        it("should revert whitelist before sale opens", async () => {
          const proof = merkleTree.getHexProof(
            ethers.keccak256(<string>purchaser?.address)
          );
          await expect(
            king
              .connect(purchaser)
              .purchaseWhitelist(proof, { value: priceEth })
          ).to.be.revertedWithCustomError(king, "SalePhaseNotStarted");
        });

        it("should revert og before sale opens", async () => {
          const proof = ogMerkleTree.getHexProof(
            ethers.keccak256(<string>accounts[4]?.address)
          );
          await expect(
            king.connect(accounts[4]).purchaseOG(proof, { value: priceEth })
          ).to.be.revertedWithCustomError(king, "SalePhaseNotStarted");
        });
      });
      describe("batch purchase", async () => {
        it("should revert open before sale opens", async () => {
          await expect(
            king.connect(purchaser).purchaseBatch(2n, { value: priceEth * 2n })
          ).to.be.revertedWithCustomError(king, "SalePhaseNotStarted");
        });
        it("should revert whitelist before sale opens", async () => {
          const proof = merkleTree.getHexProof(
          ...
          await expect(
            king
              .connect(purchaser)
              .purchaseBatchWhitelist(2n, proof, { value: priceEth * 2n })
          ).to.be.revertedWithCustomError(king, "SalePhaseNotStarted");
        });
        ...
        it("should revert og before sale opens", async () => {
          ...
          await expect(
            king
              .connect(accounts[4])
              .purchaseBatchOG(2n, proof, { value: priceEth * 2n })
          ).to.be.revertedWithCustomError(king, "SalePhaseNotStarted");
        });
      });
    });
      ...
      describe("single purchase", async () => {
        it("should revert if not enough funds", async () => {
          ...
          await expect(
            king
              .connect(accounts[4])
              .purchaseOG(proof, { value: ogPriceEth - 1n })
        ...
        it("should revert if not oged", async () => {
          ...
          await expect(
            king.connect(purchaser).purchaseOG(proof, { value: ogPriceEth })
          ).to.be.revertedWithCustomError(king, "OnlyOGWhitelist");

          // not whitelisted
          const proof2 = ogMerkleTree.getHexProof(
            ethers.keccak256(<string>accounts[6]?.address)
          );
          await expect(
            king.connect(accounts[6]).purchaseOG(proof2, { value: ogPriceEth })
          ).to.be.revertedWithCustomError(king, "OnlyOGWhitelist");
        });
        ...
        it("should accept if oged", async () => {
          ...
          await king
            .connect(accounts[4])
            .purchaseOG(proof, { value: ogPriceEth });
          expect(await king.balanceOf(<string>accounts[4]?.address)).to.equal(
            1
          );
        });
        ...
        it("should send funds to fundsCollector", async () => {
          ...
          const balanceBefore =
            await ethers.provider.getBalance(fundsCollector);
          const proof = ogMerkleTree.getHexProof(
            ethers.keccak256(<string>accounts[4]?.address)
          );
          await king
      ...
      describe("batch purchase", async () => {
        it("should revert if not enough funds", async () => {
          ...
          await expect(
            king
              .connect(accounts[4])
              .purchaseBatchOG(2n, proof, { value: ogPriceEth * 2n - 1n })
          ).to.be.revertedWithCustomError(king, "InsufficientFunds");
        });
        ...
        it("should revert if not oged", async () => {
          ...
          await expect(
            king
              .connect(purchaser)
              .purchaseBatchOG(2n, proof, { value: ogPriceEth * 2n })
          ).to.be.revertedWithCustomError(king, "OnlyOGWhitelist");
          ...
          await expect(
            king
              .connect(accounts[6])
              .purchaseBatchOG(2n, proof2, { value: ogPriceEth * 2n })
          ).to.be.revertedWithCustomError(king, "OnlyOGWhitelist");
        });
    ...
    describe("whitelist sale opens", async () => {
      ...
      describe("single purchase", async () => {
        it("should revert if not enough funds", async () => {
          ...
          await expect(
            king
              .connect(purchaser)
              .purchaseWhitelist(proof, { value: wlPriceEth - 1n })
          ).to.be.revertedWithCustomError(king, "InsufficientFunds");
        });
        it("should revert if not whitelisted", async () => {
          ...
          await expect(
            king
              .connect(accounts[6])
              .purchaseWhitelist(proof, { value: wlPriceEth })
      ...
      describe("batch purchase", async () => {
        it("should revert if not enough funds", async () => {
          ...
          await expect(
            king.connect(purchaser).purchaseBatchWhitelist(2n, proof, {
              value: wlPriceEth * 2n - 1n,
            })
          ).to.be.revertedWithCustomError(king, "InsufficientFunds");
        });
        it("should revert if not whitelisted", async () => {
          ...
          await expect(
            king
              .connect(accounts[6])
              .purchaseBatchWhitelist(2n, proof, { value: wlPriceEth * 2n })
          ).to.be.revertedWithCustomError(king, "OnlyWhitelist");
        });
    ...
    describe("open sale opens", async () => {
      ...
      describe("single purchase", async () => {
        it("should revert if not enough funds", async () => {
          await expect(
            king.connect(purchaser).purchase({ value: priceEth - 1n })
          ).to.be.revertedWithCustomError(king, "InsufficientFunds");
        });
      ...
      describe("batch purchase", async () => {
        it("should revert if not enough funds", async () => {
          await expect(
            king
              .connect(purchaser)
              .purchaseBatch(2n, { value: priceEth * 2n - 1n })
          ).to.be.revertedWithCustomError(king, "InsufficientFunds");
        });
        ...
        it("should mint multiple kings to the sender", async () => {
          await king
            .connect(purchaser)
            .purchaseBatch(2n, { value: priceEth * 2n });
          expect(await king.balanceOf(<string>purchaser?.address)).to.equal(2);
        });
        ...
        it("should revert if sold out", async () => {
          ...
          for (let i = 0n; i < amountOnSale - 1n; i += 20n) {
            const amount =
              i + 20n > amountOnSale - 1n ? amountOnSale - 1n - i : 20n;
            promises.push(
              king
                .connect(purchaser)
                .purchaseBatch(amount, { value: priceEth * amount })
            );
          }
          await Promise.all(promises);
          await expect(
  ...
  describe("implementation", async () => {
    ...
    it("expose tokenURI once minted", async () => {
      ...
      expect(await king.tokenURI(1)).to.equal(
        "https://nft.orivium.io/nft/king/1"
      );
    });
        ...
        for (let i = 0n; i < 10n * 12n; i += 12n) {
          purchasePromises.push(
            ...[
              king
                .connect(purchaser)
                .purchaseBatch(4, { value: priceEth * 4n }),
              king
                .connect(accounts[0])
                .purchaseBatch(4, { value: priceEth * 4n }),
              king
