...
import { King2d } from "@orivium/types";
...
describe("King2d ERC721", () => {
  ...
  beforeEach(async () => {
    ...
    const king2dFactory: ContractFactory =
      await ethers.getContractFactory("King2d");
    king2d = <King2d>(
      await king2dFactory.deploy(
        rootHash,
        ogRootHash,
        openMintTimestamp,
        multiSigWallet
    ...
    describe("before mint opens", async () => {
      ...
      describe("single free mint", async () => {
        ...
        it("should revert whitelist before mint opens", async () => {
          ...
          await expect(
            king2d.connect(purchaser).freeMintWhitelist(proof)
          ).to.be.revertedWithCustomError(king2d, "MintPhaseNotStarted");
        });
        ...
        it("should revert og before mint opens", async () => {
          ...
          await expect(
            king2d.connect(accounts[4]).freeMintOG(proof)
          ).to.be.revertedWithCustomError(king2d, "MintPhaseNotStarted");
        });
      });
    });
    describe("og mint opens", async () => {
      ...
      describe("single free mint", async () => {
        it("should revert if not oged", async () => {
          ...
          await expect(
            king2d.connect(purchaser).freeMintOG(proof)
          ...
          await expect(
            king2d.connect(accounts[6]).freeMintOG(proof2)
          ).to.be.revertedWithCustomError(king2d, "OnlyOGWhitelist");
        });
        ...
        it("should accept if oged", async () => {
          ...
          expect(await king2d.balanceOf(<string>accounts[4]?.address)).to.equal(
            1
          );
        });
    ...
    describe("whitelist mint opens", async () => {
      ...
      describe("single free mint", async () => {
        it("should revert if not whitelisted", async () => {
          ...
          await expect(
            king2d.connect(accounts[6]).freeMintWhitelist(proof)
        ...
        it("should accept if whitelisted", async () => {
          ...
          expect(await king2d.balanceOf(<string>purchaser?.address)).to.equal(
            1
          );
        });
    ...
    describe("open mint opens", async () => {
      ...
      describe("batch free mint", async () => {
        ...
        it("can mint a maximum of 5 king2d", async () => {
          ...
          await expect(
            king2d.connect(purchaser).freeBatchMint(1n)
          ).to.be.revertedWithCustomError(king2d, "MaxAmountMinted");
        });
        ...
        it.skip("should revert if sold out", async () => {
          ...
          await king2d.connect(purchaser).freeBatchMint(2n);
          await expect(
            king2d.connect(purchaser).freeBatchMint(2n)
          ).to.be.revertedWithCustomError(king2d, "SoldOut");
        });
      });
    });
  ...
  describe("deployment", async () => {
    it("should mint 144 nfts to the oriviumMultiSig address from 4300 to 4444", async () => {
      const kingFactory: ContractFactory =
        await ethers.getContractFactory("King2d");
      const king: King2d = <King2d>(
        await kingFactory.deploy(
          rootHash,
          ogRootHash,
          openMintTimestamp,
          multiSigWallet
  ...
  describe("implementation", async () => {
    ...
    it("expose tokenURI once minted", async () => {
      ...
      expect(await king2d.tokenURI(1)).to.equal(
        "https://nft.orivium.io/nft/king2d/1"
      );
    });
