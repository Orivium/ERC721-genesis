...
describe('MerkleTreeWhitelist', () => {
    ...
    beforeEach(async () => {
        ...
        ogMerkleTree = new MerkleTree(ogLeafNode, ethers.keccak256, { sortPairs: true });
    ...
    it("every address in the whitelist should be whitelisted", async () => {
        for (const address of whitelisted) {
            ...
            expect(await merkleTreeWhitelist.test_isWhitelisted(address, proof)).to.be.true;
        }
    });
    ...
    it("every address in the og whitelist should be whitelisted", async () => {
        for (const address of ogWhitelisted) {
            ...
            expect(await merkleTreeWhitelist.test_isOGWhitelisted(address, proof)).to.be.true;
        }
    });
    ...
    describe("modifiers", () => {
        describe("onlyOG", () => {
            it("should revert if the caller is not og whitelisted", async () => {
                if (!notWhitelisted[0]) throw new Error("notWhitelisted[0] is undefined");
                if (!whitelisted[0]) throw new Error("whitelisted[0] is undefined");
                const wlProof = merkleTree.getHexProof(ethers.keccak256(whitelisted[0]));
                const proof = ogMerkleTree.getHexProof(ethers.keccak256(notWhitelisted[0]));
                await expect(merkleTreeWhitelist.connect(
                    await ethers.getSigner(notWhitelisted[0])
                ).test_OGReserved(proof)).to.be.revertedWithCustomError(merkleTreeWhitelist, "OnlyOGWhitelist");
                await expect(merkleTreeWhitelist.connect(
                    await ethers.getSigner(whitelisted[0])
                ).test_OGReserved(wlProof)).to.be.revertedWithCustomError(merkleTreeWhitelist, "OnlyOGWhitelist");
            });
            ...
            it("should not revert if the caller is og whitelisted", async () => {
                ...
                const proof = ogMerkleTree.getHexProof(ethers.keccak256(ogWhitelisted[0]));
                await expect(merkleTreeWhitelist.connect(
                    await ethers.getSigner(ogWhitelisted[0])
                ).test_OGReserved(proof)).to.not.be.reverted;
            });
        });
        ...
        describe("onlyWhitelisted", () => {
            it("should revert if the caller is not whitelisted", async () => {
                ...
                if (!ogWhitelisted[0]) throw new Error("ogWhitelisted[0] is undefined");
                const proof = ogMerkleTree.getHexProof(ethers.keccak256(notWhitelisted[0]));
                const ogProof = ogMerkleTree.getHexProof(ethers.keccak256(ogWhitelisted[0]));
                await expect(merkleTreeWhitelist.connect(
                    await ethers.getSigner(notWhitelisted[0])
                ).test_WLReserved(proof)).to.be.revertedWithCustomError(merkleTreeWhitelist, "OnlyWhitelist");
                await expect(merkleTreeWhitelist.connect(
                    await ethers.getSigner(ogWhitelisted[0])
                ).test_WLReserved(ogProof)).to.be.revertedWithCustomError(merkleTreeWhitelist, "OnlyWhitelist");
            });
            ...
            it("should not revert if the caller is whitelisted", async () => {
                ...
                await expect(merkleTreeWhitelist.connect(
                    await ethers.getSigner(whitelisted[0])
                ).test_WLReserved(proof)).to.not.be.reverted;
            });
        });
    });
});
