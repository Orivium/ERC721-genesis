...
const deployFunction: DeployFunction = async ({
  ...
  const wlMerkleTree = new MerkleTree(wlLeafs, ethers.keccak256, {
    sortPairs: true,
  });
  ...
  const ogMerkleTree = new MerkleTree(ogLeafs, ethers.keccak256, {
    sortPairs: true,
  });
  ...
  await deploy("King", {
    from: deployer,
    args: [wlRootHash, ogRootHash, multiSigWallet],
    log: true,
  ...
  await run("verify:verify", {
    ...
    constructorArguments: [wlRootHash, ogRootHash, multiSigWallet],
  });
};
